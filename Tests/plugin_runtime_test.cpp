// Headless integration tests for the plugin layer: parameter wiring, MIDI handling,
// factory presets, bus layouts and state round-tripping. No editor is created, so this
// runs on a CI box with no display.
...
void testParameterWiring (OvertoniumProcessor& p)
{
    ...
    // Every ID the audio thread caches must actually exist in the layout. A typo here is
    // a null atomic pointer and a crash on the first block.
    ...
    for (auto* id : globals)
        check (p.apvts.getRawParameterValue (id) != nullptr, std::string ("global param ") + id);

    const char* suffixes[] = {
        ovt::params::tuneSuffix,   ovt::params::pmRateSuffix,  ovt::params::pmDepthSuffix,
        ovt::params::attackSuffix, ovt::params::decaySuffix,   ovt::params::sustainSuffix,
        ovt::params::releaseSuffix,ovt::params::amRateSuffix,  ovt::params::amDepthSuffix,
        ovt::params::muteSuffix,   ovt::params::soloSuffix,    ovt::params::volumeSuffix };

    bool allPresent = true;
    for (int i = 0; i < ovt::kNumHarmonics; ++i)
        for (auto* s : suffixes)
            allPresent &= p.apvts.getRawParameterValue (ovt::params::oscParamId (s, i)) != nullptr;
    ...
    for (int i = 0; i < ovt::kNumHarmonics; ++i)
    {
        const auto v = p.apvts.getRawParameterValue (ovt::params::oscParamId (ovt::params::volumeSuffix, i))->load();
...
void testRendering (OvertoniumProcessor& p)
{
    ...
    check (sounding.peak > 0.01f, "note is audible (peak " + std::to_string (sounding.peak) + ")");
    ...
    // Sample-accurate placement: a note at frame 256 must leave the first half silent.
    p.prepareToPlay (48000.0, 512);
...
void testPresets (OvertoniumProcessor& p)
{
    ...
    for (int i = 0; i < names.size(); ++i)
    {
        ...
        // Slow Pad and Shimmer have multi-second attacks, so give them time to open up.
        const auto stats = renderBlocks (p, 400, 512, noteOnAt (57, 1.0f, 0));

        check (stats.finite, names[i].toStdString() + ": output is finite");
        check (stats.peak > 0.005f, names[i].toStdString() + ": produces sound (peak "
                                        + std::to_string (stats.peak) + ")");
        check (stats.peak <= 1.001f, names[i].toStdString() + ": stays within full scale");
...
void testStateRoundTrip (OvertoniumProcessor& p)
{
    ...
    auto* tune7  = p.apvts.getParameter (ovt::params::oscParamId (ovt::params::tuneSuffix, 6));
    ...
    check (std::abs (p.apvts.getRawParameterValue (ovt::params::masterGainId)->load() + 3.0f) < 0.05f,
           "master survives a state round trip");
    check (std::abs (p.apvts.getRawParameterValue (ovt::params::oscParamId (ovt::params::tuneSuffix, 6))->load() - 0.25f) < 0.005f,
...
void testSoloAndMute (OvertoniumProcessor& p)
{
    ...
    // Headroom matters here: with the safety clipper engaged every variant would peak at
    // the same ceiling and the comparisons below would be meaningless.
    auto* master = p.apvts.getParameter (ovt::params::masterGainId);
    ...
    auto* mute1 = p.apvts.getParameter (ovt::params::oscParamId (ovt::params::muteSuffix, 0));
    ...
    auto* solo5 = p.apvts.getParameter (ovt::params::oscParamId (ovt::params::soloSuffix, 4));
