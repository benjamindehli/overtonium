...
//
//   c++ -std=c++17 -O2 -I Source Tests/dsp_test.cpp Source/dsp/*.cpp -o dsp_test && ./dsp_test
...
/** Single-bin DFT magnitude with a Hann window.
    ...
    The window matters: with a rectangular window the sidelobes of a strong partial only
    fall off as 1/f, which smears a neighbouring partial across the whole spectrum at
    around -48 dB and drowns out exactly the low-level alias images we are looking for.
*/
double binMagnitude (const std::vector<float>& x, double freq, double sampleRate)
{
    ...
    {
        const double win = 0.5 * (1.0 - std::cos (6.283185307179586 * (double) n / N));
...
// ---------------------------------------------------------------------------------------
// 1. The tuning table the synth derives must match the reference overtone table.
// ---------------------------------------------------------------------------------------
void testTuningTable()
{
    section ("Tuning table vs reference");

    struct Row { int n, semis, cents; const char* interval; };

    static const Row reference[32] = {
        {  1,  0,   0, "prime/octave" },  {  2, 12,   0, "prime/octave" },
        {  3, 19,   2, "fifth" },         {  4, 24,   0, "prime/octave" },
        {  5, 28, -14, "major third" },   {  6, 31,   2, "fifth" },
        {  7, 34, -31, "minor seventh" }, {  8, 36,   0, "prime/octave" },
        {  9, 38,   4, "major second" },  { 10, 40, -14, "major third" },
        { 11, 42, -49, "tritone" },       { 12, 43,   2, "fifth" },
        { 13, 44,  41, "minor sixth" },   { 14, 46, -31, "minor seventh" },
        { 15, 47, -12, "major seventh" }, { 16, 48,   0, "prime/octave" },
        { 17, 49,   5, "minor second" },  { 18, 50,   4, "major second" },
        { 19, 51,  -2, "minor third" },   { 20, 52, -14, "major third" },
        { 21, 53, -29, "fourth" },        { 22, 54, -49, "tritone" },
        { 23, 54,  28, "tritone" },       { 24, 55,   2, "fifth" },
        { 25, 56, -27, "minor sixth" },   { 26, 56,  41, "minor sixth" },
        { 27, 57,   6, "major sixth" },   { 28, 58, -31, "minor seventh" },
        { 29, 58,  30, "minor seventh" }, { 30, 59, -12, "major seventh" },
        { 31, 59,  45, "major seventh" }, { 32, 60,   0, "prime/octave" },
    };
    ...
    for (int i = 0; i < kNumHarmonics; ++i)
    {
        ...
        check (h.etSemitones == r.semis,
               "semitones for n=" + std::to_string (r.n) + " got " + std::to_string (h.etSemitones)
                   + " want " + std::to_string (r.semis));
        check ((int) std::lround (h.jiCents) == r.cents,
               "cents for n=" + std::to_string (r.n) + " got " + std::to_string (std::lround (h.jiCents))
                   + " want " + std::to_string (r.cents));
        check (std::string (intervalName (h.pitchClass)) == r.interval,
               "interval for n=" + std::to_string (r.n) + " got " + intervalName (h.pitchClass)
                   + " want " + r.interval);
    }
}

// ---------------------------------------------------------------------------------------
// 2. blend == 1 must land on an exact n:1 ratio, blend == 0 on an exact 12-TET semitone.
// ---------------------------------------------------------------------------------------
...
void testSineTable()
{
    ...
    check (std::abs ((double) sine (1.25) - (double) sine (0.25)) < 1.0e-6, "phase wraps above 1");
    check (std::abs ((double) wrapPhase (-0.25) - 0.75) < 1.0e-12, "wrapPhase handles negatives");
}
...
void testRenderedSpectrum()
{
    ...
    for (auto& o : p.osc)
        o.tuneBlend = 1.0f; // pure just intonation: partial n sits exactly at n * f0
    ...
    // Every partial up to the Nyquist guard should be present at a comparable level.
    for (int n = 1; n <= 16; ++n)
    {
        const double m = binMagnitude (l, f0 * n, sr);
        check (m > 0.3 * m1, "partial " + std::to_string (n) + " present (" + std::to_string (m / m1) + " of f0)");
    }
...
void testAliasing()
{
    ...
    check (nyquistGain (22000.0, 44100.0) == 0.0f, "partial above Nyquist silenced");
    check (nyquistGain (44100.0 * 0.455, 44100.0) > 0.0f
               && nyquistGain (44100.0 * 0.455, 44100.0) < 1.0f, "fade region is gradual");
    ...
    engine.noteOn (96, 1.0f, p); // C7 = 2093 Hz; the 32nd partial wants to sit at 67 kHz
    std::vector<float> l ((size_t) N), r ((size_t) N);
    engine.render (l.data(), r.data(), N, p);
...
void testEnvelopeAndMuteSolo()
{
    ...
    // "Decay time" means 99% of the distance covered, so two decay times in we are close
    // but the stage does not formally latch to sustain until the remainder is negligible.
    ...
    check (binMagnitude (l, f0 * 5.0, sr) < 1.0e-4, "other partials silenced by solo");
}
...
void testNoClickOnMute()
{
    ...
    engine.noteOn (45, 1.0f, p); // A2, low enough that a legitimate slope stays small
    ...
    for (int n = 1; n < N; ++n)
        steadyDelta = std::max (steadyDelta, std::abs (a[(size_t) n] - a[(size_t) n - 1]));
    ...
    std::printf ("  steady max delta %.5f, mute max delta %.5f\n", steadyDelta, muteDelta);
    check (muteDelta <= steadyDelta * 1.5f + 1.0e-4f, "muting does not introduce a step discontinuity");
...
void testVoiceAllocation()
{
    ...
    check (engine.getActiveVoiceCount() <= SynthEngine::kPoolSize, "pool is never over-subscribed");

    // Let the stolen tails finish, then only the polyphony limit should still be held.
    for (int i = 0; i < 40; ++i)
        engine.render (l.data(), r.data(), N, p);

    check (engine.getActiveVoiceCount() <= 4,
           "polyphony limit respected after fades (" + std::to_string (engine.getActiveVoiceCount()) + " active)");
    ...
    engine.noteOn (60, 1.0f, p);
    engine.render (l.data(), r.data(), N, p);
    check (engine.getActiveVoiceCount() == after1, "repeated note-on reuses the same voice");

    // Sustain pedal holds released notes.
    engine.allSoundOff();
    p.osc[0].release = 0.05f; // the 2 s release set above would outlast this sub-test
    ...
    for (int i = 0; i < 200; ++i)
        engine.render (l.data(), r.data(), N, p);
    check (engine.getActiveVoiceCount() == 0, "releasing the pedal releases the note");
}
...
void benchmark()
{
    ...
    for (int poly : { 1, 8, 16 })
    {
        ...
        std::printf ("  %2d voices (%3d partials): %.3f s for %.0f s of audio  ->  %.2f%% of one core\n",
                     poly, poly * kNumHarmonics, elapsed, secs, load);

        check (engine.getActiveVoiceCount() == poly, "all benchmark voices sounding");
    }
}
