...
namespace
{
    juce::NormalisableRange<float> rangeWithCentre (float lo, float hi, float centre)
    {
    ...
    {
        if (v <= 0.0005f) return "Equal temp.";
        if (v >= 0.9995f) return "Just";
...
{
    // 1/n over the first eight partials, silence above: a soft sawtooth / drawbar blend
    // that is immediately playable without being a wall of sound.
    if (index0 >= 8)
        return 0.0f;
...
juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
{
    ...
    // ---- global ------------------------------------------------------------------------
    layout.add (std::make_unique<FloatP> (juce::ParameterID { masterGainId, 1 },
                                          "Master",
    ...
    layout.add (std::make_unique<juce::AudioParameterChoice> (juce::ParameterID { polyphonyId, 1 },
                                                              "Polyphony",
                                                              polyChoices,
                                                              4)); // 8 voices

    layout.add (std::make_unique<FloatP> (juce::ParameterID { spreadId, 1 },
                                          "Stereo Spread",
                                          juce::NormalisableRange<float> (0.0f, 1.0f),
                                          0.0f,
                                          FAttr().withStringFromValueFunction (percentText)));

    layout.add (std::make_unique<FloatP> (juce::ParameterID { velAmountId, 1 },
                                          "Velocity Amount",
                                          juce::NormalisableRange<float> (0.0f, 1.0f),
                                          0.7f,
                                          FAttr().withStringFromValueFunction (percentText)));

    layout.add (std::make_unique<juce::AudioParameterInt> (juce::ParameterID { bendRangeId, 1 },
                                                           "Pitch Bend Range",
                                                           0, 24, 2));
    ...
    for (int i = 0; i < kNumHarmonics; ++i)
    {
        ...
        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (tuneSuffix, i), 1 },
                                              p + "Tune",
                                              juce::NormalisableRange<float> (0.0f, 1.0f),
                                              1.0f, // the pure harmonic series is the natural home position
                                              FAttr().withStringFromValueFunction (blendText)));

        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (pmRateSuffix, i), 1 },
                                              p + "Pitch Mod Rate",
                                              rangeWithCentre (0.01f, 30.0f, 2.0f),
                                              4.0f,
                                              FAttr().withLabel ("Hz")));

        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (pmDepthSuffix, i), 1 },
                                              p + "Pitch Mod Depth",
                                              rangeWithCentre (0.0f, 200.0f, 25.0f),
                                              0.0f,
                                              FAttr().withLabel ("ct")));

        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (attackSuffix, i), 1 },
                                              p + "Attack",
                                              rangeWithCentre (0.0005f, 5.0f, 0.03f),
                                              0.005f,
                                              FAttr().withStringFromValueFunction (timeText)));

        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (decaySuffix, i), 1 },
                                              p + "Decay",
                                              rangeWithCentre (0.001f, 20.0f, 0.5f),
                                              0.6f,
                                              FAttr().withStringFromValueFunction (timeText)));

        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (sustainSuffix, i), 1 },
                                              p + "Sustain",
                                              juce::NormalisableRange<float> (0.0f, 1.0f),
                                              1.0f,
                                              FAttr().withStringFromValueFunction (percentText)));

        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (releaseSuffix, i), 1 },
                                              p + "Release",
                                              rangeWithCentre (0.001f, 20.0f, 0.5f),
                                              0.4f,
                                              FAttr().withStringFromValueFunction (timeText)));

        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (amRateSuffix, i), 1 },
                                              p + "Amp Mod Rate",
                                              rangeWithCentre (0.01f, 30.0f, 4.0f),
                                              4.0f,
                                              FAttr().withLabel ("Hz")));

        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (amDepthSuffix, i), 1 },
                                              p + "Amp Mod Depth",
                                              juce::NormalisableRange<float> (0.0f, 1.0f),
                                              0.0f,
                                              FAttr().withStringFromValueFunction (percentText)));

        layout.add (std::make_unique<BoolP> (juce::ParameterID { oscParamId (muteSuffix, i), 1 },
                                             p + "Mute", false));

        layout.add (std::make_unique<BoolP> (juce::ParameterID { oscParamId (soloSuffix, i), 1 },
                                             p + "Solo", false));

        // Square-law fader: half travel lands at a quarter of full amplitude, which is
        // roughly where the ear expects "half as loud".
        layout.add (std::make_unique<FloatP> (juce::ParameterID { oscParamId (volumeSuffix, i), 1 },
                                              p + "Level",
...
void Cache::snapshot (SynthParams& out, float bendNormalised) const
{
    ...
    out.global.masterGain    = juce::Decibels::decibelsToGain (masterGain->load(), -60.0f);
