...
void LabelledKnob::paint (juce::Graphics& g)
{
    ...
    g.drawText (caption, getLocalBounds().removeFromBottom (11), juce::Justification::centred, false);
}
...
TopBar::TopBar (juce::AudioProcessorValueTreeState& state, juce::Component& popupParent)
    : apvts (state)
{
    ...
    tuneAll.slider.setTooltip ("Sweeps every partial between equal temperament and just intonation");
    tuneAll.slider.onUserDragStart = [this] { if (onTuneAllDragStart) onTuneAllDragStart(); };
    tuneAll.slider.onUserDragEnd   = [this] { if (onTuneAllDragEnd)   onTuneAllDragEnd(); };
    ...
    masterAttachment = std::make_unique<SliderAttachment> (apvts, params::masterGainId, master.slider);
    spreadAttachment = std::make_unique<SliderAttachment> (apvts, params::spreadId,     spread.slider);
    velAttachment    = std::make_unique<SliderAttachment> (apvts, params::velAmountId,  velocity.slider);
    bendAttachment   = std::make_unique<SliderAttachment> (apvts, params::bendRangeId,  bend.slider);
    ...
    polyAttachment = std::make_unique<ComboBoxAttachment> (apvts, params::polyphonyId, polyBox);
    polyBox.setTooltip ("Maximum simultaneous notes. Each note runs 32 sine partials.");
    addAndMakeVisible (polyBox);

    // ---- zoom --------------------------------------------------------------------------
    for (int i = 0; i < (int) std::size (kZoomChoices); ++i)
        zoomBox.addItem (juce::String (juce::roundToInt (kZoomChoices[i] * 100.0f)) + "%", i + 1);
    ...
    // ---- toggles -----------------------------------------------------------------------
    styleToggle (linkButton,  "LINK",  "Gang every strip: moving one knob moves all 32");
    styleToggle (phaseButton, "PHASE", "Reset partial phase on each note for a coherent attack");
    styleToggle (clipButton,  "CLIP",  "Soft-clip the output. Worth leaving on with 32 faders.");

    linkButton.setColour (juce::TextButton::buttonOnColourId, colours::soloOn);

    phaseAttachment = std::make_unique<ButtonAttachment> (apvts, params::phaseResetId, phaseButton);
    clipAttachment  = std::make_unique<ButtonAttachment> (apvts, params::safetyClipId, clipButton);

    // ---- captions ----------------------------------------------------------------------
    struct { juce::Label* label; const char* text; juce::Justification just; } captions[] = {
...
void TopBar::styleToggle (juce::TextButton& b, const juce::String& text, const juce::String& tooltip)
{
...
{
    voicesLabel.setText (juce::String (active) + " / " + juce::String (limit) + " voices",
...
void TopBar::paint (juce::Graphics& g)
{
    ...
    g.drawText ("OVERTONIUM", title.removeFromTop (24), juce::Justification::centredLeft, false);
...
void TopBar::resized()
{
    ...
    auto placeCaptioned = [] (juce::Component& c, juce::Label& cap, juce::Rectangle<int> column)
    {
    ...
    // Everything on the right is placed from the right edge inwards, so the knob cluster
    // in the middle is what gets squeezed when the window is narrow.
    voicesLabel.setBounds (b.removeFromRight (juce::jmin (72, juce::jmax (0, b.getWidth()))));
    b.removeFromRight (6);
    ...
    if (b.getWidth() > 110)
        placeCaptioned (presetBox, presetCaption, b.removeFromRight (juce::jmin (170, b.getWidth())));
}
