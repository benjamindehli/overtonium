...
/** A Slider that knows whether the human is currently holding it.
    ...
    That distinction is what makes the LINK switch safe: a value change caused by a drag
    gets broadcast to the other 31 strips, while one caused by automation, a preset load
    or the broadcast itself does not, so there is no feedback loop.
*/
...
private:
    ...
    LinkableSlider tune, pmRate, pmDepth, attack, decay, sustain, release, amRate, amDepth, volume;
