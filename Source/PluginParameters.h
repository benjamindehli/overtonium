...
/** Default fader level for a partial: a 1/n roll-off over the first octave-and-a-bit,
    which lands somewhere between a drawbar organ and a soft sawtooth. */
float defaultVolumeFor (int index0);

/** Cached atomic pointers, resolved once so the audio thread never does a string lookup. */
struct Cache
{
    struct Osc
    {
    ...
    /** Builds an audio-thread snapshot. @param bendNormalised is -1..1 from the MIDI wheel. */
