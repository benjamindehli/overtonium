...
/** Linearly interpolated sine lookup table.
    ...
    A 4096-point table with linear interpolation keeps harmonic distortion around
    -95 dBFS, which is inaudible here and roughly an order of magnitude cheaper than
    calling std::sin once per sample. With 32 partials x 16 voices that difference is
    the whole ball game.
*/
class SineTable
{
...
private:
    SineTable() noexcept
    {
        for (int i = 0; i <= kSize; ++i)
            tbl[(size_t) i] = (float) std::sin (6.283185307179586476 * (double) i / (double) kSize);
    }

    std::array<float, kSize + 1> tbl {}; // guard point at the end makes interpolation branch-free
};
