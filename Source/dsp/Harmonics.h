...
/** Tuning data for one partial of the harmonic series.
    ...
    Everything is derived from the harmonic number n rather than hard-coded, so the
    just-intonation frequency is exact (blend == 1 gives precisely n * fundamental).
    Rounded to whole cents this reproduces the classic overtone deviation table:
...
struct HarmonicInfo
{
    ...
    double exactCents  = 0.0; ///< 1200 * log2(n): the true just interval above the fundamental
    int    etSemitones = 0;   ///< nearest 12-TET semitone
    double jiCents     = 0.0; ///< signed cent deviation of the just interval from etSemitones
...
inline const std::array<HarmonicInfo, kNumHarmonics>& harmonicTable() noexcept
{
    static const std::array<HarmonicInfo, kNumHarmonics> table = []
    {
        ...
        for (int i = 0; i < kNumHarmonics; ++i)
        {
            ...
            t[(size_t) i].etSemitones = (int) std::lround (t[(size_t) i].exactCents / 100.0);
            t[(size_t) i].jiCents     = t[(size_t) i].exactCents - 100.0 * t[(size_t) i].etSemitones;
...
/** @param index0  zero-based partial index (0 == fundamental, 31 == 32nd harmonic). */
...
/** Semitone offset above the played note.
    ...
    @param blend  0 == equal temperament (snapped to the nearest semitone),
                  1 == just intonation (an exact n:1 ratio with the fundamental).
*/
...
{
    static const char* names[12] = { "prime/octave", "minor second", "major second", "minor third",
