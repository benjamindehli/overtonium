...
**Two kinds of lit button, and the difference is what the light is for.** A mute or a solo lights its whole face, because the question it answers is asked across 33 channels at once and the eye has to find it without reading anything. A switch that stands on its own, which is ECHO, REVERB, LINK and the character, does the opposite: the face is drawn exactly as it is when the switch is off, down to the shade of grey, and the word itself comes up lit with its light spilling onto the face around it. There is only ever one of those in view, so it has nothing to be found among, and a panel of faces lighting up in four colours would be a fairground.
